...
const (
	defaultSessionRuntimeTTL      = 30 * time.Minute
	sessionRuntimeJanitorInterval = 1 * time.Minute
)
...
type sessionRuntime struct {
	...
	fingerprint string
	modelName   string
	agent       sortedagents.Agent
	runner      sortedagents.Runner
	cleanups    []func()
	runMu       sync.Mutex
...
func (r *sessionRuntime) touch() {
	r.lastUsedNs.Store(time.Now().UnixNano())
}

func (r *sessionRuntime) lastUsedTime() time.Time {
	lastUsed := r.lastUsedNs.Load()
	if lastUsed == 0 {
		return time.Time{}
	}
...
func (r *sessionRuntime) close() {
	r.cleanupOnce.Do(func() {
		// Ensure cleanup does not run while the runtime is actively executing.
		r.runMu.Lock()
...
func NewAgentService(daoFactory db.DAOFactory, settingsManager *settings.SettingsManager) (*AgentServiceAPI, error) {
	...
	svc := &AgentServiceAPI{
		...
		runtimeTTL:      defaultSessionRuntimeTTL,
		runtimes:        make(map[string]*sessionRuntime),
	}
	svc.startRuntimeJanitor()
	return svc, nil
}
...
func (s *AgentServiceAPI) UpdateAgent(ctx context.Context, req *pb.UpdateAgentRequest) (*pb.UpdateAgentResponse, error) {
	...
	s.invalidateRuntimesByAgent(req.AgentId)
...
func (s *AgentServiceAPI) AgentChat(req *pb.AgentChatRequest, stream pb.AgentService_AgentChatServer) error {
	...
	if err := s.runAgentWithCallbacks(ctx, req.SessionId, *agentRow, messages, req.Message, stream, &nextSeq); err != nil {
...
func (s *AgentServiceAPI) runAgentWithCallbacks(
	...
	runtime, err := s.getOrCreateSessionRuntime(ctx, sessionID, agentRow, providerSettings)
	if err != nil {
		return err
	}
...
func (s *AgentServiceAPI) startRuntimeJanitor() {
	if s.runtimeTTL <= 0 {
		return
	}
	go func() {
		ticker := time.NewTicker(sessionRuntimeJanitorInterval)
		defer ticker.Stop()
		for range ticker.C {
...
func (s *AgentServiceAPI) evictExpiredRuntimes() {
	now := time.Now()
	var expired []*sessionRuntime

	s.runtimesMu.Lock()
	for sessionID, runtime := range s.runtimes {
		lastUsed := runtime.lastUsedTime()
		if lastUsed.IsZero() || now.Sub(lastUsed) > s.runtimeTTL {
	...
	for _, runtime := range expired {
		slog.Info("Evicting expired agent runtime", "sessionID", runtime.sessionID, "agentID", runtime.agentID)
		runtime.close()
	}
}

func (s *AgentServiceAPI) invalidateRuntime(sessionID string, reason string) {
	var runtime *sessionRuntime
	s.runtimesMu.Lock()
	if existing, ok := s.runtimes[sessionID]; ok {
		delete(s.runtimes, sessionID)
		runtime = existing
	}
	s.runtimesMu.Unlock()
	...
	if runtime != nil {
		slog.Info("Invalidating agent runtime", "sessionID", sessionID, "agentID", runtime.agentID, "reason", reason)
		runtime.close()
	}
}

func (s *AgentServiceAPI) invalidateRuntimesByAgent(agentID string) {
	...
	for _, runtime := range stale {
		slog.Info("Invalidating agent runtime due to agent update", "sessionID", runtime.sessionID, "agentID", agentID)
		runtime.close()
	}
}

func (s *AgentServiceAPI) computeRuntimeFingerprint(agentRow db.AgentRow, providerSettings *pb.ProviderSettings) string {
	var apiURL, apiKey string
	var isEnabled bool
	if providerSettings != nil {
		apiURL = providerSettings.GetApiUrl()
		apiKey = providerSettings.GetApiKey()
		isEnabled = providerSettings.GetIsEnabled()
	}
...
func (s *AgentServiceAPI) getOrCreateSessionRuntime(
	ctx context.Context,
	sessionID string,
	agentRow db.AgentRow,
	providerSettings *pb.ProviderSettings,
) (*sessionRuntime, error) {
	fingerprint := s.computeRuntimeFingerprint(agentRow, providerSettings)

	s.runtimesMu.RLock()
	existing := s.runtimes[sessionID]
	if existing != nil && existing.agentID == agentRow.ID && existing.fingerprint == fingerprint {
		existing.touch()
	...
	newRuntime, err := s.buildSessionRuntime(ctx, sessionID, agentRow, providerSettings, fingerprint)
	if err != nil {
		return nil, err
	}
	...
	existing = s.runtimes[sessionID]
	if existing != nil {
		if existing.agentID == agentRow.ID && existing.fingerprint == fingerprint {
			s.runtimesMu.Unlock()
			newRuntime.close()
			existing.touch()
			return existing, nil
	...
	s.runtimes[sessionID] = newRuntime
	s.runtimesMu.Unlock()

	if stale != nil {
		slog.Info("Replacing stale agent runtime", "sessionID", sessionID, "agentID", stale.agentID)
		stale.close()
	}
...
func (s *AgentServiceAPI) buildSessionRuntime(
	ctx context.Context,
	sessionID string,
	agentRow db.AgentRow,
	providerSettings *pb.ProviderSettings,
	...
	slog.Info("Creating cached agent runtime", "model", modelName, "agentID", agentRow.ID, "sessionID", sessionID)
	...
	slog.Debug("Added tools to cached runtime", "count", len(tools), "workspace", workspacePath, "mcpServers", len(mcpServers))
