...
func NewSQLiteDAO(sqliteUrl string) (*SQLiteDAO, error) {
	...
	if err != nil {
		slog.Error("RealtimeService:dao_sqlite:NewSQLiteDAO", "error", "failed to open SQLite database", "error", err)
	...
	if err != nil {
		slog.Error("RealtimeService:dao_sqlite:NewSQLiteDAO", "error", "failed to set busy timeout", "error", err)
		return nil, err
	}
	...
	if err != nil {
		slog.Error("RealtimeService:dao_sqlite:NewSQLiteDAO", "error", "failed to set WAL mode", "error", err)
		return nil, err
	}
...
func (d *SQLiteDAO) CreateAudioChat(userID string, modelName string, startTime string, endTime string) (string, error) {
	...
	if err != nil {
		slog.Error("RealtimeService:dao_sqlite:CreateAudioChat", "error", "failed to create audio chat", "error", err)
...
func (d *SQLiteDAO) UpdateAudioChat(userID string, id string, endTime string) error {
	...
	if err != nil {
		slog.Error("RealtimeService:dao_sqlite:UpdateAudioChat", "error", "failed to update audio chat", "error", err)
