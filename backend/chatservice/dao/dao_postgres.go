...
func (p *PostgresDAO) AddChatMessage(userID string, chatId string, role string, content string, contentImage string, model string, inputTokens int, outputTokens int, cachedTokens int, references string, ragEnabled bool) (string, error) {
	...
	// Insert into DB with content_image (NULL if empty)
...
func (p *PostgresDAO) AddChatMessageWithTokens(
	...
	sqlQuery := `
		...
		calc AS (
			SELECT (p.input_token_cost * $6
				...
				+ p.cached_token_cost * $8) / 1000000.0 + $14 AS cost
			FROM prices p
		),
		upd_msg AS (
	...
	err := p.db.QueryRow(sqlQuery,
		...
		userID, referencesValue, ragEnabled, braveSearchCount, scrapeAPIUsageTime, searchCost,
	).Scan(&messageId, &cost)
...
func (p *PostgresDAO) IsNameExists(userID string, chatId string, name string) (bool, error) {
	slog.Info("dao_postgres:IsNameExists", "userID", userID, "chatId", chatId, "name", name)
	var exists bool
	err := p.db.Get(&exists, "SELECT EXISTS(SELECT 1 FROM chat_list WHERE name = $1 AND user_id = $2 AND chat_id != $3 AND soft_deleted = FALSE)", name, userID, chatId)
	if err != nil {
		slog.Error("dao_postgres:IsNameExists", "message", "failed to check if name exists", "error", err, "userID", userID, "chatId", chatId, "name", name)
