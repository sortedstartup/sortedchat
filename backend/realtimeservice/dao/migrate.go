...
const MIGRATION_TABLE = "realtimeservice_migrations"
const SEED_MIGRATION_TABLE = "realtimeservice_seed"
...
func MigrateSQLite(dbURL string) error {
	slog.Info("RealtimeService: Migrating SQLite database", "dbURL", dbURL)
	sqlite_vec.Auto()
...
func SeedSqlite(dbURL string) error {
	slog.Info("RealtimeService: Seeding SQLite database", "dbURL", dbURL)
	sqlite_vec.Auto()
	sqlDB, err := sql.Open("sqlite3", dbURL)
	if err != nil {
		slog.Error("error", "err", err)
...
func MigratePostgres(dbURL string) error {
	slog.Info("RealtimeService: Connecting to PostgreSQL database")
...
func SeedPostgres(dbURL string) error {
	slog.Info("RealtimeService: Seeding PostgreSQL database", "dbURL", dbURL)
	sqlDB, err := sql.Open("postgres", dbURL)
	if err != nil {
		slog.Error("error", "err", err)
		return err
