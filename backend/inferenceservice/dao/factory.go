...
func NewDAOFactory(config *Config) (DAOFactory, error) {
	...
	case DatabaseTypeSQLite:
		...
		if err != nil {
			slog.Error("inferenceservice:dao:NewDAOFactory", "message", "failed to open SQLite database", "error", err)
			return nil, err
		}
		...
		if err != nil {
			slog.Error("inferenceservice:dao:NewDAOFactory", "message", "failed to set busy timeout", "error", err)
			return nil, err
		}
		...
		if err != nil {
			slog.Error("inferenceservice:dao:NewDAOFactory", "message", "failed to set WAL mode", "error", err)
			return nil, err
		}
	...
	case DatabaseTypePostgres:
		...
		if err != nil {
			slog.Error("inferenceservice:dao:NewDAOFactory", "message", "failed to open PostgreSQL connection", "error", err)
		...
		if err := db.Ping(); err != nil {
			db.Close()
			slog.Error("inferenceservice:dao:NewDAOFactory", "message", "failed to ping PostgreSQL database", "error", err)
	...
	default:
		slog.Error("inferenceservice:dao:NewDAOFactory", "message", "unsupported database type", "database_type", config.Database.Type)
...
func (f *SQLiteDAOFactory) CreateDAO() (DAO, error) {
	...
	return NewSQLiteDAOWithDB(f.db)
}
